...
public class SafeArea : MonoBehaviour
{
    ...
    public Rect GetSafeArea()
    {
        ...
        {
            Rect newSafeArea = new Rect(0, 0, Screen.width, Screen.height);

            switch (Sim)
            {
                case SimDevice.iPhoneX:
                    if (Screen.height > Screen.width) // Portrait
                        newSafeArea = NSA_iPhoneX[0];
                    else // Landscape
                        newSafeArea = NSA_iPhoneX[1];
                    break;
                case SimDevice.iPhoneXsMax:
                    if (Screen.height > Screen.width) // Portrait
                        newSafeArea = NSA_iPhoneXsMax[0];
                    else // Landscape
                        newSafeArea = NSA_iPhoneXsMax[1];
                    break;
                case SimDevice.Pixel3XL_LSL:
                    if (Screen.height > Screen.width) // Portrait
                        newSafeArea = NSA_Pixel3XL_LSL[0];
                    else // Landscape
                        newSafeArea = NSA_Pixel3XL_LSL[1];
                    break;
                case SimDevice.Pixel3XL_LSR:
                    if (Screen.height > Screen.width) // Portrait
                        newSafeArea = NSA_Pixel3XL_LSR[0];
                    else // Landscape
                        newSafeArea = NSA_Pixel3XL_LSR[1];
                    break;
                default:
                    break;
            }

            safeArea = new Rect(Screen.width * newSafeArea.x, Screen.height * newSafeArea.y,
                Screen.width * newSafeArea.width, Screen.height * newSafeArea.height);
        }
